...
export default function PlayerColumn({ guesses = [], opponentName = "Opponent" }) {
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ fontSize: '0.82rem', fontWeight: '700', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
          My Guesses at {opponentName}'s Number
        </span>
      ...
      <div style={{ flex: 1, overflowY: 'auto', maxHeight: '280px' }}>
        {guesses.length === 0 ? (
          <div style={{
            ...
            <Target size={24} style={{ opacity: 0.4 }} />
            <span>No guesses made yet. Take your shot!</span>
          </div>
        ) : (
                ...
                return (
                  <tr
                    ...
                    style={{
                      borderBottom: '1px solid var(--border-subtle)',
                      textAlign: 'center',
                      background: isWin ? 'var(--pos-match-bg)' : 'transparent',
                    }}
                  >
